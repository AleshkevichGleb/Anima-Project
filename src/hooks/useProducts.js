...
export const FilterAndSearchedProducts = (products, sort, price, brand, color, productMM) => {
    const sortedProducts = useMemo(() => {
        ...
        const filter = {brand, color, productMM};
        let newState = [...products];

        for(let item in filter) {
            
            if(item  === 'brand' &&  Object.keys(brand).length) {
                let brandArr = [];
            ...
            if(item  === 'color' && Object.keys(color).length) {
                let colorArr = [];
            ...
            if(item  === 'productMM' &&  Object.keys(productMM).length) {
                let mmArr = [];
